...
class CNN(object):
    ...
    def process(self):
        self.X_train_image, self.X_test_image, self.y_train_label, self.y_test_label = train_test_split(self.arr_x,
                                                                                                        self.arr_y,
                                                                                                        test_size=0.3,
                                                                                                        random_state=42)
        self.y_trainOneHot, self.y_testOneHot = np_utils.to_categorical(self.y_train_label), np_utils.to_categorical(
            self.y_test_label)
    ...
    def build(self):
        ...
        self.model.add(Conv2D(filters=32, #32
                              kernel_size=(2, 2),
                              padding='same',
                              activation='relu'))
        self.model.add(MaxPooling2D(pool_size=(2, 2)))
    ...
    def show(self):
        ...
        self.show_train_history(self.train_history, 'precision', 'val_precision')
        self.show_train_history(self.train_history, 'recall', 'val_recall')
